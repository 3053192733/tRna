...
export function cleanSequence(input: string): string {
  return input.toUpperCase().replace(/[^ATGCU]/g, '').trim();
}
...
export function getSequenceStats(sequence: string): { length: number; gcContent: number; counts: Record<string, number> } {
  ...
  return { length: cleaned.length, gcContent: isNaN(gc) ? 0 : gc, counts };
}
