...
export function calculateDNAMolecularWeight(
  sequence: string,
  isDoubleStranded: boolean = false
): number {
...
export function calculateProteinMolecularWeight(sequence: string): {
  mw: number;
  details: { aminoAcid: string; mw: number }[];
} {
...
export function calculateConcentration(
  ...
  dilutionFactor: number = 1
): number {
  const concentration = (absorbance / extinctionCoefficient) * dilutionFactor * 1000000;
  return Math.round(concentration * 1000) / 1000;
}

export function calculateOD260(
  concentration: number,
  extinctionCoefficient: number,
  dilutionFactor: number = 1
): number {
  return (concentration / 1000000) * extinctionCoefficient / dilutionFactor;
}

export function formatMolecularWeight(mw: number): string {
  if (mw >= 1000000) {
    return `${(mw / 1000000).toFixed(2)} MDa`;
  } else if (mw >= 1000) {
